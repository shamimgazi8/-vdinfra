...
// Define the shape of the preset filter options
type FilterPreset = "all_time" | "last_8h" | "last_24h" | "last_7d" | "last_30d" | "this_month" | "custom";
...
export const DateFilterPopover = ({
  ...
  // Function to set the current time to the end time input
  ...
  // New function to format the time range
  const formatTimeRange = () => {
    // Check if the time has been customized from the default
  ...
  // Logic to determine the combined button text
  const getButtonText = () => {
    const dateText = formatDateRange();
    const timeText = formatTimeRange();

    // If no custom dates or times are set, use the default
    if (dateText === "Created At" && timeText === "All Day") {
      return "Created At";
    }

    // Combine date and time if a date range is selected
    if (dateRange?.from) {
  ...
  return (
    <Popover open={open} onOpenChange={setOpen} >
      <PopoverTrigger asChild>
        <Button
          variant="outline"
          className={cn("bg-white text-black border hover:bg-black hover:text-white")}
        >
          ...
          <div className="flex w-64 flex-col p-4 border-r">
            {filterPresets.map((preset) => (
              <Button
                ...
                className={cn(
                  "justify-start mb-2",
                  selectedPreset === preset.value && "bg-gray-100 font-semibold"
                )}
              >
                {preset.label}
              </Button>
            ...
            <div className="mt-4 border-t pt-4">
              <div className="flex justify-between items-center mb-4">
                <div className="flex-1 mr-2">
                  ...
                  <Input type="text" value={startTime} onChange={(e) => setStartTime(e.target.value)} className="mt-1" />
                </div>
                <div className="flex-1 ml-2">
                  <label className="text-sm font-medium flex justify-between">
                    End Time
                    <span
                      className="text-sm font-medium text-[#16A34A] ml-4 cursor-pointer pb-1"
                      onClick={setCurrentTime}
                    >
                      Current Time
                    </span>
                  </label>
                  <Input type="text" value={endTime} onChange={(e) => setEndTime(e.target.value)} className="mt-1" />
                </div>
              </div>
